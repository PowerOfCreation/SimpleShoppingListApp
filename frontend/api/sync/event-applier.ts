 ...
 * ahead of what the projection actually reflects.
 *
 ...
 * one hand-managed transaction rather than owning a single table, and
 ...
 * called from inside apply()'s transaction is therefore one that takes an
      ...
      await runExclusive(() =>
        this.db.withTransactionAsync(async () => {
          ...
          if (applied > 0) {
            ...
            // If the merged history's last word for this list is
            // todo_list.deleted, the list projection just deleted the row -
            // rebuilding ingredients would re-insert rows referencing a
            // list that no longer exists in the read model. Harmless
            // on-device (expo-sqlite runs without PRAGMA foreign_keys) but
            // a hard FK violation under the exclusive-FK test mock, and
            // pointless either way - just drop them instead.
            const listRow = await this.db.getFirstAsync<{ id: string }>(
              `SELECT id FROM ingredient_lists WHERE id = ?`,
              listId
            )
            if (listRow) {
              await this.ingredientProjection.rebuildForList(
                this.db,
                listId,
                history
              )
