...
func applySchema(ctx context.Context, conn *pgx.Conn) error {
	...
	migrations := []string{
		"00001-initial-schema.up.sql",
		"00002-events-table.up.sql",
		"00003-events-processed-at.up.sql",
	}

	for _, file := range migrations {
		schemaBytes, err := os.ReadFile(filepath.Join(projectRoot, "migrations", file))
		if err != nil {
